...
@Service
public class ValidateEmailLatestConfirmationUseCaseImpl implements ValidateEmailLatestConfirmationUseCase {

    private static final Logger LOGGER = LoggerFactory.getLogger(ValidateEmailLatestConfirmationUseCaseImpl.class);

    private final UserRepository repository;
    private final VerificationTokenService verificationTokenService;

    public ValidateEmailLatestConfirmationUseCaseImpl(final UserRepository repository,
    ...
    public VerificationAttemptResponseDTO execute(final UUID userId, final String code) {
        ...
        LOGGER.info("Validating email confirmation against the latest token for user {}", user.id());
        return verificationTokenService.validateLatestToken(user, VerificationChannel.EMAIL, code);
    }
}
