...
@Service
public class ConfirmEmailByTokenUseCaseImpl implements ConfirmEmailByTokenUseCase {

    private static final Logger LOGGER = LoggerFactory.getLogger(ConfirmEmailByTokenUseCaseImpl.class);

    private final UserRepository repository;
    private final VerificationTokenService verificationTokenService;

    public ConfirmEmailByTokenUseCaseImpl(final UserRepository repository,
    ...
    public void execute(final UUID userId, final String token) {
        ...
        LOGGER.info("Confirming email contact for user {} using verification token", user.id());
        verificationTokenService.confirmToken(user, VerificationChannel.EMAIL, token);
    }
}
